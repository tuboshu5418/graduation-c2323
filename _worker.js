export default {
  async fetch(request, env) {
    ...
    if (path === '/login' && request.method === 'POST') {
      ...
      if (result) {
        return Response.json({ success: true, user: { id: result.id, name: result.name, role: result.role, avatar: result.avatar || '' } }, { headers: corsHeaders });
      }
    ...
    if (path === '/change-password' && request.method === 'PUT') {
      ...
      const user = await env.DB.prepare('SELECT id FROM classmates WHERE name = ? AND password_hash = ?').bind(name, oldHash).first();
      if (!user) return Response.json({ success: false, error: '原密码错误' }, { status: 401, headers: corsHeaders });
      const newHash = await sha256(newPassword);
      await env.DB.prepare('UPDATE classmates SET password_hash = ? WHERE name = ?').bind(newHash, name).run();
    ...
    if (path === '/classmates-list' && request.method === 'GET') {
      const result = await env.DB.prepare("SELECT name FROM classmates WHERE role IN ('student','admin') ORDER BY name").all();
      return Response.json(result.results, { headers: corsHeaders });
    }
    
    // ========== 获取同学 ==========
    if (path === '/classmates' && request.method === 'GET') {
      const result = await env.DB.prepare("SELECT id, name, avatar FROM classmates WHERE role = 'student' ORDER BY name").all();
      return Response.json(result.results, { headers: corsHeaders });
    }
    
    // ========== 获取老师 ==========
    if (path === '/teachers' && request.method === 'GET') {
      const result = await env.DB.prepare("SELECT id, name, role, avatar, subject FROM classmates WHERE role = 'teacher' ORDER BY CASE subject WHEN '语文' THEN 1 WHEN '数学' THEN 2 WHEN '英语' THEN 3 WHEN '物理' THEN 4 WHEN '化学' THEN 5 WHEN '政治' THEN 6 WHEN '历史' THEN 7 WHEN '音乐' THEN 8 WHEN '体育' THEN 9 WHEN '美术' THEN 10 ELSE 99 END, name").all();
      return Response.json(result.results, { headers: corsHeaders });
    }
    ...
    if (path.startsWith('/contact/') && request.method === 'GET') {
      ...
      const result = await env.DB.prepare('SELECT phone, email, wechat, motto, role, avatar, subject FROM classmates WHERE name = ?').bind(name).first();
    ...
    if (path === '/admin/add-user' && request.method === 'POST') {
      ...
      await env.DB.prepare('INSERT INTO classmates (name, password_hash, role, phone, email, wechat) VALUES (?, ?, ?, ?, ?, ?)').bind(name, defaultHash, role, phone, email, wechat).run();
      return Response.json({ success: true }, { headers: corsHeaders });
    }
    ...
    if (path.startsWith('/messages/') && request.method === 'GET') {
      ...
      const result = await env.DB.prepare('SELECT id, from_name, content, created_at FROM messages WHERE to_name = ? ORDER BY created_at DESC').bind(toName).all();
      return Response.json(result.results, { headers: corsHeaders });
    }
    ...
    if (path.startsWith('/feedback/') && request.method === 'GET') {
      ...
      let params = [toName];
      if (type !== 'all') { query += ' AND type = ?'; params.push(type); }
    ...
    if (path === '/admin/all-data' && request.method === 'GET') {
      ...
      const user = await env.DB.prepare("SELECT role FROM classmates WHERE name = ? AND role = 'admin'").bind(requester).first();
      if (!user) return Response.json({ error: '权限不足' }, { status: 403, headers: corsHeaders });
    ...
    if (path === '/admin/query' && request.method === 'POST') {
      ...
      if (upperSql.includes('DROP') || upperSql.includes('DELETE') || upperSql.includes('INSERT') || upperSql.includes('UPDATE') || upperSql.includes('ALTER') || upperSql.includes('CREATE')) {
        return Response.json({ error: '仅允许 SELECT' }, { status: 400, headers: corsHeaders });
      }
      try { const result = await env.DB.prepare(sql).all(); return Response.json({ results: result.results }, { headers: corsHeaders }); }
    ...
    if (path === '/photos' && request.method === 'GET') {
      const result = await env.DB.prepare('SELECT id, uploaded_by, title, description, image_url, created_at FROM photos ORDER BY created_at DESC').all();
      return Response.json(result.results, { headers: corsHeaders });
    }
    if (path.startsWith('/photos/') && request.method === 'DELETE') {
      const photoId = path.replace('/photos/', '');
      const { requester } = await request.json();
      const photo = await env.DB.prepare('SELECT uploaded_by FROM photos WHERE id = ?').bind(photoId).first();
      if (!photo) return Response.json({ error: '照片不存在' }, { status: 404, headers: corsHeaders });
      const user = await env.DB.prepare("SELECT role FROM classmates WHERE name = ?").bind(requester).first();
      if (photo.uploaded_by !== requester && user?.role !== 'admin') return Response.json({ error: '权限不足' }, { status: 403, headers: corsHeaders });
    ...
    if (path === '/signature-wall' && request.method === 'GET') {
      const result = await env.DB.prepare('SELECT image_data, updated_at FROM signature_wall WHERE id = 1').first();
      return Response.json(result ? { image_data: result.image_data, updated_at: result.updated_at } : { image_data: null }, { headers: corsHeaders });
    }
    ...
    if (path === '/songs' && request.method === 'GET') {
      const result = await env.DB.prepare('SELECT id, name, filename, uploaded_by, created_at FROM songs ORDER BY created_at DESC').all();
      return Response.json(result.results, { headers: corsHeaders });
    }
    ...
    if (path === '/songs/upload' && request.method === 'POST') {
      ...
      if (!file || !songName) return Response.json({ error: '缺少参数' }, { status: 400, headers: corsHeaders });
      if (!/^[a-zA-Z0-9_\-\u4e00-\u9fa5]+$/.test(songName)) return Response.json({ error: '非法字符' }, { status: 400, headers: corsHeaders });
      if (file.size > 20*1024*1024) return Response.json({ error: '文件过大' }, { status: 400, headers: corsHeaders });
      const arrayBuffer = await file.arrayBuffer();
      const base64 = btoa(String.fromCharCode(...new Uint8Array(arrayBuffer)));
      const fileName = file.name || `${songName}.${fileType}`;
      const filePath = `songs/${songName}/${fileName}`;
      ...
      const body = { message: `上传: ${songName} - ${fileName}`, content: base64, branch: 'main' };
      if (sha) body.sha = sha;
      try {
        ...
        if (gr.ok) return Response.json({ success: true, path: filePath }, { headers: corsHeaders });
        return Response.json({ error: gd.message || '失败' }, { status: 400, headers: corsHeaders });
    ...
    if (path === '/drift-bottles/sent' && request.method === 'GET') {
      ...
      const result = await env.DB.prepare('SELECT id, from_name, to_name, content, open_time, is_opened, created_at FROM drift_bottles WHERE from_name = ? ORDER BY created_at DESC').bind(name).all();
      return Response.json(result.results, { headers: corsHeaders });
    }
    if (path === '/drift-bottles/inbox' && request.method === 'GET') {
      const name = new URL(request.url).searchParams.get('name');
      if (!name) return Response.json({ error: '缺少姓名' }, { status: 400, headers: corsHeaders });
      const result = await env.DB.prepare("SELECT id, from_name, to_name, content, open_time, is_opened, created_at FROM drift_bottles WHERE to_name = ? AND open_time <= datetime('now') ORDER BY created_at DESC").bind(name).all();
      return Response.json(result.results, { headers: corsHeaders });
    }
